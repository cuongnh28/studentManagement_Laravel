...
<div class="form">
    <h1>Sửa thông tin</h1>
    <form name="edit_student" action="{{ route('updateTeacher') }}" method="post">
        @csrf
        <input type="text" name="id" value="{{$getTeacherById[0]->id}}"
               hidden>
        <input type="text" name="username" placeholder="Tên đăng nhập" value="{{$getTeacherById[0]->username}}"
               disabled>
        <input type="text" name="name" placeholder="Họ và tên" value="{{$getTeacherById[0]->name}}"
               disabled>
        <input type="email" name="email" placeholder="Email" value="{{$getTeacherById[0]->email}}">
        <input type="text" name="phone" placeholder="Số điện thoại" value="{{$getTeacherById[0]->phone}}">
        <input type="password" name="password" placeholder="Mật khẩu" value="{{$getTeacherById[0]->password}}"> <br><br>
